...
这是 Vue 2 源码阅读系列的第 3 篇，重点理解当新旧 VNode 出现差异时，Vue 如何用 patch 和 diff 把真实 DOM 更新成本压到更低。

## 阅读提示

- 本篇默认你已经知道 VNode 是什么
- 阅读重点放在 patch 主流程和 `updateChildren` 的比较策略
- 先理解“同层比较”和“双端比较”，再看具体源码会轻松很多
...
- Vue 更新视图不是整树重建，而是围绕 patch 做差异更新
- `sameVnode` 决定了节点能否复用
- `updateChildren` 是列表 diff 的核心位置，`key` 在这里非常重要

## 继续阅读

- 上一篇：[VNode节点](/analysis/vue/2)
- 下一篇：[Vue.js异步更新DOM策略及nextTick](/analysis/vue/4)
