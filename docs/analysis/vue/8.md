...
这是 Vue 2 源码阅读系列的第 8 篇，重点理解 Vuex 为什么能接管复杂状态，以及它如何借助 Vue 的响应式系统把全局状态、getter、mutation、action 串起来。

## 阅读提示

- 建议先看完组件通信，再进入这一篇
- 阅读重点放在 `install`、`Store`、`installModule`、`resetStoreVM`
- 这篇信息量最大，先抓主骨架，再回头看 API 细节更合适
...
这一篇可以把 Vuex 拆成三条主线来看：

- 注入：通过 `install` 和 `vuexInit` 把 `$store` 放到所有组件实例上
- 组织：通过 `installModule` 把模块、getter、mutation、action 注册起来
- 响应式：通过 `resetStoreVM` 借助 Vue 实例让 store 具备响应式更新能力

## 继续阅读

- 上一篇：[Vue组件间通信](/analysis/vue/7)
- 返回栏目：[源码阅读](/analysis/)
