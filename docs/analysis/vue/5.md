...
这是 Vue 2 源码阅读系列的第 5 篇，重点理解为什么不是所有数据变化都会触发整页重渲染，以及 Vue 是如何把“谁依赖了谁”这件事记录下来的。

## 阅读提示

- 这篇和响应式原理紧密相关，可以一起看
- 重点理解 `Dep`、`Watcher` 和 `Dep.target` 三者关系
- 本篇先抓住最小模型，后续再把它和渲染更新流程合起来看
...
- 为什么模板里没用到的数据，不应该触发无意义更新
- `getter` 为什么不仅是“取值”，还是依赖收集入口
- `Dep` 和 `Watcher` 怎样形成订阅关系

## 继续阅读

- 上一篇：[Vue 异步更新 DOM 与 nextTick](/analysis/vue/4)
- 下一篇：[Vue事件机制](/analysis/vue/6)
