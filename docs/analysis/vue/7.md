...
这是 Vue 2 源码阅读系列的第 7 篇，重点从实际通信方式切入，梳理父子、兄弟、跨层级组件之间有哪些常见数据传递手段，以及各自的适用边界。

## 阅读提示

- 这篇偏工程实践总结，是前几篇源码机制在组件层的落地
- 阅读时重点区分“推荐方式”和“可以做到但不推荐的方式”
- 如果你准备进入 Vuex，这篇正好能解释为什么简单通信方案会在复杂应用里失控
...
- 简单父子通信优先 `props` 和 `$emit`
- 一旦状态共享开始跨层、跨区域扩散，就要考虑更稳定的状态管理方案

## 继续阅读

- 上一篇：[Vue事件机制](/analysis/vue/6)
- 下一篇：[Vuex源码解析](/analysis/vue/8)
