...
这是 Vue 2 源码阅读系列的第 1 篇，重点理解 Vue 为什么能在数据变化后自动更新视图，以及 `data` 是怎样一步步变成“可观察对象”的。

## 阅读提示

- 建议先带着“修改数据后，视图为什么会变”这个问题进入
- 本篇先抓主流程，不必一开始就深究所有边界情况
- 如果你后面要读依赖收集、Watcher、nextTick，这篇是前置基础
...
- Vue 2 的响应式核心依赖 `Object.defineProperty`
- Vue 不只把数据做成可观察对象，还会把 `data` 代理到实例上，降低访问成本

## 继续阅读

- 下一篇：[VNode节点](/analysis/vue/2)
