# Vue 异步更新 DOM 与 nextTick

这是 Vue 2 源码阅读系列的第 4 篇，重点理解为什么你改完数据后立刻拿到的 DOM 还是旧值，以及 Vue 为什么要把更新视图放到异步队列里做。

## 阅读提示

- 这篇是把响应式和视图更新时机连接起来的关键一环
- 重点关注 `queueWatcher`、`nextTick` 和 `flushSchedulerQueue`
- 阅读时始终带着“为什么不能同步立即更新”这个问题
...
- Vue 默认不会在每次数据变更后立即同步更新 DOM
- Watcher 会先进队列，在下一个 tick 统一刷新
- `nextTick` 的价值就是让你在 DOM 真正更新完成后再读取结果

## 继续阅读

- 上一篇：[VirtualDOM与diff](/analysis/vue/3)
- 下一篇：[依赖收集](/analysis/vue/5)
